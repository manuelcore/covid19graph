...
function processCSVData(allText) {
	...
	//input: judet/tara, data (format:day/month/year), carantinati, infectati, decedati, vindecati
	//         0                   1                  2           3         4           5
	...
	$.each( coordCountyState, function( i1, v1 ){
		if(v1[0]==splitdata[0]){lat=v1[1];lon=v1[2];}
	});
    ...
    for (var i=0;i<parseInt(splitdata[targetColumn], 10);i++)
	{
		...
		$.each( dataHeatmap, function( i2, v2 ){
			if(v2[0]==splitdata[1]){v2[1].push(L.latLng(lat, lon));found=1;}
		});
		//add if new date
		//if(found==0)dataHeatmap.push([splitdata[1],[lat, lon]] );
